...
import { soufflets, meatOptions, sauceOptions, garnitureOptions, menuOptionPrices } from '@/data/menu';
...
export function SouffletWizard({ onClose }: SouffletWizardProps) {
  ...
  const calculatePrice = () => {
    ...
    // Add garniture supplements
    selectedGarnitures.forEach(garId => {
      const gar = garnitureOptions.find(g => g.id === garId);
      if (gar) price += gar.price;
    });
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {garnitureOptions.map((gar) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{gar.name}</span>
                    {selectedGarnitures.includes(gar.id) && <Check className="w-5 h-5 text-primary" />}
                  </div>
                  {gar.price > 0 && (
                    <span className="text-sm text-primary font-semibold">+{gar.price}€</span>
                  )}
                </Card>
              ))}
            </div>
