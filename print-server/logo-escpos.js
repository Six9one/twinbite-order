...
// Cache dynamic logos to avoid fetching/processing on every print job
...
export async function buildLogoBytesFromUrl(url, targetWidth = 160, isStar = false) {
    ...
    try {
        ...
        const aspect = img.height / img.width;
        const targetH = Math.ceil(width * aspect);
        img = img.resize({ w: width, h: targetH });
        ...
        console.log(`[LOGO] Custom logo compiled successfully (${buffer.length} bytes)`);
        return buffer;
    } catch (err) {
        console.warn(`[LOGO] Failed to build custom logo from URL "${url}":`, err.message);
        return null;
    }
}
